...
def get_share_value_from_13F(company_name : str, company_ticker : str , financial_institution_name : str, year : int, aggregation_method = sum) -> float:
    DIR_13F = './data/13f_data'

    company_directory = os.path.join(DIR_13F, financial_institution_name)
    return get_share_value_from_directory(company_name, company_ticker, company_directory, year, aggregation_method=aggregation_method)

def get_share_value_from_directory(company_name: str, company_ticker: str, directory: str, year: int, aggregation_method = sum)->float:
    aggregated_so_far = 0
    for filename in os.listdir(directory):  
        filepath = os.path.join(directory, filename)
        if os.path.isfile(filepath) and str(year) in filename:
            with open(filepath, mode='r', newline='') as source_file:
                ...
                if header[0] == 'Manager':
                    ...
                    fin = False
            if fin:
                aggregated_so_far = aggregation_method(get_share_value_from_fin_csv(company_name, filepath), aggregated_so_far)
            else:
                aggregated_so_far = aggregation_method(get_share_value_from_management_csv(company_ticker, filepath), aggregated_so_far)
        elif os.path.isdir(filepath):
            aggregated_so_far = aggregation_method(get_share_value_from_directory(company_name, company_ticker, filepath, year), aggregated_so_far)
...
def get_share_value_from_fin_csv(company_name: str, path_to_csv: str) -> float:
    return 0
    with open(path_to_csv, mode='r', newline='') as source_file:
        ...
        return sum(safe_to_float(row[3], 0) for row in reader if company_name.lower() in row[0].lower())
...
if __name__ == "__main__":
    ...
    for i in range(len(company_names)):
        ...
        for bank in banks:
            all_shares = []
            for year in years:
                all_shares.append(get_share_value_from_13F(company_name, company_ticker, bank, year))
